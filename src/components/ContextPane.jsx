...
export function ContextPane({ worktree, settings, onError }) {
  ...
  if (!context) return <div className="pane-loading" role="status" aria-live="polite"><SpinnerGap className="spin" />Reading project context…</div>;
  return <section className="context-pane-content">
    <ContextSection icon={BookOpen} title="Project instructions" count={context.instructionFiles.length}>{context.instructionFiles.length ? context.instructionFiles.map((file) => <article className="context-card" key={file.path}><header><strong>{file.name}</strong><Button variant="ghost" size="icon-xs" aria-label={`Open ${file.name} in ${settings.editor}`} onClick={() => api("/api/editor/open", { method: "POST", body: { path: worktree.path, file: file.path.replace(`${worktree.path}/`, ""), editor: settings.editor } }).catch(onError)}><ArrowSquareOut /></Button></header><pre tabIndex={0} aria-label={`Preview of ${file.name}`}>{file.preview}</pre></article>) : <p>No AGENTS.md, CLAUDE.md, or Copilot instructions found.</p>}</ContextSection>
