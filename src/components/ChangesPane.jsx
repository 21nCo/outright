  ...
  return <section className="changes-pane">
    ...
    <div className="changes-layout">
      <div className="changed-files">
        {!status?.files.length && <div className="clean-state"><Check />Working tree is clean</div>}
        {status?.files.map((file) => <div key={file.path} className={`change-file ${file.path === selectedFile ? "is-active" : ""}`} role="button" tabIndex={0} onClick={() => chooseFile(file)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); chooseFile(file); } }}><span className={`file-status ${hasStaged(file) ? "staged" : ""}`}>{file.status}</span><span>{file.path}</span><span className="file-actions">{hasStaged(file) ? <Button variant="ghost" size="icon-xs" onClick={(event) => { event.stopPropagation(); mutate("/api/git/unstage", [file.path]); }} aria-label={`Unstage ${file.path}`}><Minus /></Button> : <Button variant="ghost" size="icon-xs" onClick={(event) => { event.stopPropagation(); mutate("/api/git/stage", [file.path]); }} aria-label={`Stage ${file.path}`}><Plus /></Button>}<Button variant="ghost" size="icon-xs" onClick={(event) => { event.stopPropagation(); api("/api/editor/open", { method: "POST", body: { path: worktree.path, file: file.path, editor: settings.editor } }).catch(onError); }} aria-label={`Open ${file.path}`}><ArrowSquareOut /></Button></span></div>)}
      </div>
      <div className="diff-column">
        {selected && <div className="diff-mode" role="group" aria-label="Diff view"><Button variant={viewMode === "unstaged" ? "secondary" : "ghost"} size="xs" onClick={() => chooseMode("unstaged")}>Unstaged</Button><Button variant={viewMode === "staged" ? "secondary" : "ghost"} size="xs" disabled={!stagedEligible} onClick={() => chooseMode("staged")}>Staged{selected.originalPath ? ` (renamed from ${selected.originalPath})` : ""}</Button></div>}
        <pre className="diff-view">{diff ? diff.split("\n").map((line, index) => <span className={line.startsWith("+") && !line.startsWith("+++") ? "added" : line.startsWith("-") && !line.startsWith("---") ? "removed" : line.startsWith("@@") ? "hunk" : ""} key={`${index}:${line}`}><i>{index + 1}</i>{line}{"\n"}</span>) : <span className="diff-empty">Select a changed file to inspect its diff.</span>}</pre>
      </div>
    </div>
    <footer className="commit-bar"><Input value={commitMessage} onChange={(event) => setCommitMessage(event.target.value)} placeholder="Commit message" onKeyDown={(event) => { if (event.key === "Enter") commit(); }} /><Button onClick={commit} disabled={!commitMessage.trim() || !status?.stagedCount}><GitCommit /> Commit {status?.stagedCount ? `${status.stagedCount} staged` : ""}</Button></footer>
  </section>;
}
